...
def cmd_evaluate_notion_idea(url: str | None, dry_run: bool = False) -> int:
    """Evaluate a single pasted Notion idea page against the Storelli brain.
    Writes the evaluation artifact to ADHOC_IDEA_EVALUATIONS unless --dry-run;
    Notion and canonical evidence (internal rows, profiles, inspiration, semantic
    connections, calendar ratings) are never written. External inspiration is a
