...
def _mode_ideas(user_text: str) -> str:
    ...
    # Honour an explicitly requested count here too — this is the fallback path
    # used when no RATED ideas exist yet, and it had the same bug as the rated
    # path: an ask for 10 was silently answered with 5.
    import idea_retrieval as _ir
    asked = _ir._asked_count((user_text or "").lower())
    ideas = interpretation.build_idea_candidates(
        question=user_text, rows=rows, findings=results, context=ctx,
        limit=asked or 5)
    if not ideas:
        return _NO_DATA_MSG
    body = _render_ideas(ideas, analyzed, buckets)
    if asked and len(ideas) < asked:
        # Name the real constraint: these are generated by pairing winning hooks
        # with winning formats, so the signal set is the ceiling — not a cap we
        # chose. Saying so beats quietly returning fewer.
        body = (f"You asked for {asked}; the current winning-signal set only "
                f"supports {len(ideas)} distinct grounded "
                f"{'idea' if len(ideas) == 1 else 'ideas'} — more tagged videos "
                f"would widen it.\n\n" + body)
